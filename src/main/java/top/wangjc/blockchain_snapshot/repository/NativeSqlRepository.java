...
public class NativeSqlRepository {
    ...
    public int batchInsertEthAccount(List<EthAccountEntity> accounts) {
        ...
        accounts.forEach(account -> insertSql.append(String.format("('%s','%s','%s'),", account.getAddress(), account.getBalance().toString(), account.getUsdtBalance().toString())));
    ...
    public int batchInsertEthTrx(List<EthTrxEntity> trxs) {
        ...
        trxs.forEach(trx -> insertSql.append(String.format("('%s','%s',%d,%d,'%s','%s','%s'),", trx.getTrxHash(), trx.getBlockHash(), trx.getBlockNumber(), trx.getTrxIndex(), trx.getTrxFrom(), trx.getTrxTo(), trx.getTrxValue().toString())));
        insertSql.deleteCharAt(insertSql.lastIndexOf(","));
        Query insertQuery = entityManager.createNativeQuery(insertSql.toString());
        return insertQuery.executeUpdate();
    }
